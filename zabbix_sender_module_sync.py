"""
Drop-in single-file synchronous Zabbix sender module.
"""
...
class Sender:
    ...
    def _parse_response_header(header: bytes) -> tuple[int, bool]:
        ...
        return data_len_low + (data_len_high << 32), bool(flags & 0x02)
    ...
    def _open_socket(self) -> socket.socket:
        raw_sock = socket.create_connection((self.server, self.port), timeout=self.timeout)
        if self.tls_connect == "psk":
            ctx = self._create_psk_ssl_context()
            return ctx.wrap_socket(
                raw_sock,
                server_hostname=self.tls_server_name or self.server,
            )
        return raw_sock

    def send(self, items=None) -> ZabbixResponse:
        ...
        try:
            sock.sendall(packet)
            header = self._read_exact(sock, 13)
            resp_size, compressed = self._parse_response_header(header)
            body = self._read_exact(sock, resp_size)
            if compressed:
    ...
    def send_safe(self, items=None) -> SendResult:
        try:
            return SendResult(ok=True, response=self.send(items))
        except Exception as exc:
...
def send_safe(
    ...
    try:
        ...
        return sender.send_safe(items)
    except Exception as exc:
        return SendResult(ok=False, error=_format_sender_error(exc))


__all__ = ["ItemData", "ZabbixResponse", "SendResult", "Sender", "send_safe"]
